...
- 理符“歼敌指令：冰霜龙鸟”接取和交付 NPC 已按实测修正为洛蒂耶（双蛇党牙士长），库尔札斯中央高地 `(11.9, 16.8)`。
- `ZodiacMonsterTracker` 优先读取聊天事件的 `Message.TextValue`，并解析黄道文书系统提示中的 `目标名 当前数/总数` 直接同步进度；例如“讨伐合成矿妖虫 3/3”会直接完成目标。无法解析数字时才按旧战斗文本递增 1。
- 黄道武器 iLvl 125 阶段已录入四条材料任务的 16 个指定副本；每行支持独立完成勾选和 `AD执行`，`zodiac-zodiac-quests` 由四个副本分组完成状态自动计算。
- 新增 `DutyCompletionTracker`：按已确认的 `TerritoryType.RowId` 统一监听 `IDutyState.DutyCompleted`，自动标记古武黄道文书 27 个副本、黄道武器四任务 16 个副本和幻武秘影 35 个副本；古武使用 UI 当前选择职业，幻武使用全局完成集，只在游戏成功完成副本时标记。
- 古武悬浮窗的“黄道”阶段同时展示四任务各自 `x/4`，并为每个未完成组提供下一个副本的 `AD执行`。
...
- 装备列表中装备名支持左键打开 Wiki、右键菜单“打开 Wiki / 检索 / 复制道具名 / 删除”；装备位置表列宽由 ImGui 自动保存。
- BIS 设置页支持按物品 ID 和按物品名称两种批量添加，名称按客户端 `Item` 表精确匹配；右键“删除”会从当前列表移除并清理全局物品项。
- 旧版分享链接（版本 5 及以下）拒绝导入并提示在配装器重新生成（编辑-分享-复制）；`EquipSlotCategory` 字段为 `sbyte`，判定用 `> 0`。
