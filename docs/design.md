...
- `Phantom.csproj`：Dalamud API 15 插件项目配置，当前发布目标版本 0.1.37.0。
- `Phantom.json`：卫月插件清单（含 IconUrl，当前发布目标 AssemblyVersion 0.1.37.0）。
...
- 武器同步除扫描当前可读取的物品外，还读取内置成就映射对应的 `Achievement.RowId`；当前物品命中或历史成就已解锁均判定为该职业阶段曾经拥有。成就来源在阶段卡片上显示“成就”，悬停提示显示“成就已解锁，判定为曾经拥有”，不伪装成当前 Item.RowId。
...
- 曼武、雅武、幻武的武器总进度页，以及绝武“总进度”页底部显示“低保数量”卡片。低保固定使用客户端 `Item.RowId`，不按本地化名称匹配；设置页同步工具提供“导出低保 Item.RowId”用于重新核对客户端数据。阶段材料按累计阶段计算：已完成更高阶段的武器也视为已完成此前阶段。
- 曼武低保：稀少陨石 `38420`、稀少球粒陨石 `38940`、稀少无球粒陨石 `40322`、雏晶 `41032`。每把武器需要 3 个，显示 `当前持有/剩余需求`；剩余需求为 `(全部职业数 - 对应阶段累计完成数) × 3`。
- 幻武低保：新月矿石 `47750`（半影）、上弦月矿石 `46850`（本影）、下弦月矿石 `50058`（黯影）、朔月矿石 `50977`（蚀影）。每把武器需要 3 个，显示 `当前持有/剩余需求`；更高阶段已完成的武器会累计视为此前阶段已完成，例如半影完成 7 把时新月矿石剩余需求为 `(21 - 7) × 3 = 42`。
- 绝武低保：龙神图腾 `21197`、究极图腾 `23175`、机神城图腾 `28633`、龙诗图腾 `36810`、欧米茄图腾 `38951`、巫女图腾 `44743`、小丑图腾 `52321`。每个绝本独立计算，显示 `当前持有/该绝本未完成职业数`；绝武总进度页显示全部七种图腾，各绝本单独页签底部只显示本绝本对应的图腾。
- 雅武低保：全天强化药 `41699`、阿罗阿罗岛书籍 `41080`、科尔沃黄铜币 `50847`、科尔沃书籍 `49125`。按剩余优雅阶段武器数计算需求；兑换规则为 1 本阿罗阿罗岛书籍、40 枚科尔沃黄铜币或 20 本科尔沃书籍兑换 1 个全天强化药，卡片同时显示原材料需求和可兑换强化药数量。
- 低保数量按当前已加载的背包、鞍囊和雇员库存堆叠实时汇总；未打开过的雇员缓存不保证完整。高品质物品 ID 先规范化再计数。
...
- 水天文书第一卷理符“焚书任务：回收禁书«黑暗羽翼的怪物»”接取 NPC 为瓦力诺，库尔札斯中央高地 `(12.6, 16.7)`；不可回退到洛蒂耶 `(11.9, 16.8)`。
- 理符“歼敌指令：冰霜龙鸟”接取和交付 NPC 为洛蒂耶（双蛇党牙士长），库尔札斯中央高地 `(11.9, 16.8)`。
...
- 黄道武器 iLvl 125 阶段录入“滴水之恩”“挑战未知的工匠”“激情与梦想”“慈母手中物”四条任务及其 16 个指定副本。每个副本独立勾选并提供 `AD执行`；四任务进度由各组 4 个副本是否全部完成自动计算，不再手填。
- iLvl 125 阶段的 4 个任务副本数据位于 `ZodiacGuide.ZodiacZodiacDutyGroups`，每个副本的完成 Key 以任务分组 Key 和序号组成，并写入当前角色/职业的 `ZodiacJobProgress.CompletedObjectives`。
- 古武悬浮监控选择“黄道”阶段时同时显示四条任务：顶部显示总进度 `x/4`，每组显示副本进度 `x/4`；未完成组的 `AD执行` 自动启动该组下一个未完成副本，已完成组显示“完成”。
- “滴水之恩”：山中战线泽梅尔要塞、纷争要地布雷福洛克斯野营地、魔兽领域日影地修炼所、凛冽洞天披雪大冰壁。
- “挑战未知的工匠”：毒雾洞窟黄金谷、恶灵府邸静语庄园、腐坏遗迹无限城市街古迹、逆转要害沙斯塔夏溶洞。
- “激情与梦想”：神灵圣域放浪神古神殿、骚乱坑道铜铃铜山、财宝传说破舰岛、苏醒遗迹喀恩埋没圣堂。
- “慈母手中物”：邪教驻地无限城古堡、领航明灯天狼星灯塔、惨剧灵殿塔姆·塔拉墓园、激战城塞石卫塔。
...
BIS 联动是左侧导航“妖表联动”下方的正式栏目（图标 `Table`），不通过隐藏命令进入；原 `/肝武 隐藏栏目` 入口已移除。栏目包含“装备列表”“装备套装”“设置”“检索”四个页签。

装备列表：

- 每个装备列表拥有独立名称和一组 `Item.RowId`，与旧版全局 `TrackedEquipment` 分离。
- 由 BIS 套装保存时可选择生成一个同名装备列表。
- 列表页选择列表后直接显示该列表的装备位置表格，复用自定义位置同步逻辑。
- 列表名称与同名 BIS 套装匹配时显示“打开分享网页”，打开该套装保存的分享链接。
- 新建列表必须先输入名称；默认列表 `default` 不允许删除。
- 设置页的批量导入、导入、导出、雾忆批量和移除都作用于当前选中的装备列表。
- 设置页支持按物品 ID 和按物品名称两种批量添加；名称按客户端 `Item` 表精确匹配转成 ID，未匹配的名称保留并提示。
- 装备列表中的装备名支持左键打开物品 Wiki；右键菜单提供“打开 Wiki”“检索”“复制道具名”和“删除”，删除会从当前列表移除并清理全局物品项。

装备套装：

- 套装保存名称、职业、来源分享链接和部位映射 `Slots`。
- 部位顺序固定为：武器、头部、身体、手部、腿部、脚部、耳饰、项链、手镯、戒指1、戒指2、食物。
- 主手和副手统一显示为“武器”；旧数据中的“主手”键在读取时兼容。
- 解析预览和已保存套装都按上述顺序展示，未识别部位显示“未识别”。
- 每个已保存套装提供“打开分享网页”和“删除”。
- 顶部提供“BIS大全”按钮，打开 <https://www.kdocs.cn/l/ceEcTzlFQBUy>。
- 测试按钮会填入示例分享链接、套装名称“绝妖武士”和职业 `SAM`。

BIS 分享链接解析：

- 使用本地 `Features/BIS/BISShareDecoder.cs`，不请求 ffxiv-gearing 站点脚本。
- 读取 URL 第一个 `?` 之后的 token，按自定义 Base62 字母表 `0-9a-zA-Z` 还原为 `BigInteger`。
- 按上游版本 6+ 的混合进制顺序读取版本、职业、等级、同步等级、装备编码类型、魔晶石编码表、装备 ID。
- 版本低于 6、职业越界、未知字符或魔晶石编码表为空时拒绝解析并提示。
- 普通装备、特殊装备、可定制装备和戒指顺序均按上游字段规则处理；戒指保留两枚。
- 解析出的 Item ID 通过客户端 `Item` 表校验，命中后才映射部位；未命中计入未知 ID 提示。
- 部位映射读取 `EquipSlotCategory` 的 `MainHand`、`Head`、`Body`、`Gloves`、`Legs`、`Feet`、`Ears`、`Neck`、`Wrists`、`FingerL`、`FingerR` 字段。
- `EquipSlotCategory` 字段类型为 `sbyte`，判定必须使用 `> 0`，不能直接当布尔值。
- 分享 token 不包含食物，食物需要单独录入或加入装备列表；未识别的食物归入最后的“食物/其他”区域。
- 解析成功后显示职业、等级和 12 部位预览；未解析成功时不允许保存套装。

位置同步：

- “同步当前角色”只扫描当前选中装备列表中的物品。
- 扫描范围与自定义位置表一致：背包、兵装库、装备中、陆行鸟鞍囊、投影台、收藏柜和雇员缓存。
- 鞍囊、投影台、收藏柜和雇员结果依赖客户端已加载缓存；未加载的雇员显示“未加载”，不等同于没有。
- 位置快照按角色保存到 `TrackedEquipmentLocationsByCharacter`，同步时间保存到 `TrackedEquipmentSyncTimesByCharacter`。
- 表格列宽允许鼠标拖拽，由 ImGui 按固定表格 ID 自动保存；设置页不再提供手写行高和列宽的输入框。
- 装备列表中的装备名支持左键打开物品 Wiki；右键菜单提供“打开 Wiki”“检索”“复制道具名”和“删除”。
- “检索”当前执行 `/isearch 物品名` 并输出插件提示。由于 `ICommandManager.ProcessCommand` 无法触发游戏原生指令，该功能暂时无效，待定方案为引入 `ECommons` 依赖或改用宏系统。
- 旧版分享链接（版本 5 及以下）不支持导入，提示“请在配装器重新生成分享链接（编辑-分享-复制）”，不做字段猜测解析。
- “背包优先”只会先按装备部位排序，再在同一部位内把背包中的物品提前；不会把背包中的食物移动到武器之前。
...
- 左侧导航栏分为“武器工坊”和“Tools”两组，列宽按图标、最长系列名称和右侧计数动态计算，避免高 DPI 或字体缩放下文字与数量重叠。
...
- 同步工具卡片的调试按钮按当前可用宽度自动换行，按钮宽度按可见文字计算；高 DPI 或窄窗口下不得用无条件 `SameLine` 横向挤压。
...
- 狩猎助手始终保存车头最后一个有效 Flag。战斗中收到新 Flag 时立即更新游戏地图 Flag、停止旧导航并覆盖待执行目标，不尝试上坐骑或传送；脱战后只执行最后收到的 Flag。不同坐标的新 Flag 不受 5 秒重复过滤影响。
- 跨地图 Flag 的 Lifestream 传送如果未开始，狩猎导航立即返回并明确提示，不再把异地图世界坐标交给当前地图 vnavmesh。
...
- 悬浮窗狩猎助手卡片显示最新 Flag 坐标和“等待脱战”状态，并提供“前往最新 Flag”按钮；未收到有效 Flag 时按钮禁用。手动点击与自动处理共用同一队列，战斗中点击同样等待脱战后执行。
...
- 悬浮窗可自由拉大拉小：不再使用自动适应尺寸，默认尺寸 400×420，窗口带最小尺寸约束（340×180），内容超出窗口高度时出现滚动条；幻武监控、狩猎助手、危命助手三张卡片宽度跟随窗口宽度。
- 悬浮窗各卡片高度按当前字体行高计算：折叠高度使用 `GetFrameHeightWithSpacing()`，展开高度按内容行数估算；可能超出高度的卡片允许内部滚动，不使用固定 22/34/72px 这类裸高度压缩文本。
- 悬浮窗标题按钮高度使用当前 frame 高度。标题行右侧的“停止导航”“展开/收起”等按钮通过安全右对齐放置：空间足够时同行靠右，空间不足时自动换行，避免覆盖状态文字。
- 古武悬浮监控的职业和阶段下拉框按可用宽度分配，空间不足时改为分行显示；“下一步”内容使用换行文本，不再在 `TextWrapped` 后强行追加同行按钮。
